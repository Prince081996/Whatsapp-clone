...
function Sidebar() {
    ...
    const createChat = () => {
        ...
        if (EmailValidator.validate(input) && !chatAlreadyExits(input) && input !== user.email) {
            // we need to add te chat into the Db 'chats collection and we also want to user login
    ...
    const chatAlreadyExits = (recepientEmail) => 
    ...
    return (
        <div>
            <Container>
                ...
                <Search>
                    <SearchIcon />
                    <input style={{outlineWidth:0,border:"none",flex:1}} placeholder="Search in Chats"  />
                </Search>
...
const SidebarButton = styled(Button)`
 width:100%;
 &&& {
     border-top:1 px solid whitesmoke;
